...
【やることメモ優先順位順】
（最優先）・トレーリングが不要なコマンドの整理（JOIN,PART、KICK,PING、MODE、INVITE、NICK、TOPIC あたりを調査。）
・mode b のブロードキャスト（決めた戦略で行く。ただし、RFC 絡み）
・BJ のロジックの修復、計算、乱数、保有。
・オペレーターはチャンネルに必ず必要か
・議論がわかれるとろこに、方針の決定（POLL の 0、）
・チンチロで、班長一味が 456 確定じゃなくて、456 賽を使うように修正する。
