...
・レジスター前のユーザーにもブロードキャストがされる。
・
・

【やったこと】
・トピックの極めて簡素な一般的機能のテスト
→ セットユーザーと時間に関連するメンバの追加と、RFC2814 のトピック削除を NC で対応それに伴い、パースにトレイリングフラグメンバ追加。
→ モードがチャンネル指定しないとユーザーモードの扱いになるが、モードはチャンネル固有指定なので
　対応せず、221 のユーザーモードサポートのフラグより 501 の ERR を返答するように。
...
cap,invite,join,kick,mode,names,nick,part,pass,ping,privmsg,quit,user,utils,who,whois
topic,
